        ...
        >
          {collections.map((item) => (
            <div
              ...
              style={{ width: '100vw' }}
            >
              <div className="absolute left-4 md:left-12 top-24 md:bottom-12 font-header text-[120px] md:text-[220px] leading-none text-deep-walnut/[0.06] pointer-events-none">
                0{item.id}
              </div>

              <div className="flex flex-col lg:flex-row items-center justify-center gap-6 lg:gap-16 w-full max-w-7xl mx-auto z-10">
                <div className="w-2/3 sm:w-1/2 lg:w-1/2 relative group max-h-[45vh] lg:max-h-none flex justify-center" style={{ perspective: '1000px' }}>
                  <div className="relative w-full max-w-[280px] lg:max-w-none aspect-[4/5] rounded-xl border border-deep-walnut/20 bg-gradient-to-br from-void-start to-void-end transform-style-3d shrink-0">
                    {/* The Product Inside */}
                    <motion.div
                ...
                <div className="w-full lg:w-1/2 flex flex-col items-center text-center lg:items-start lg:text-left gap-4 lg:gap-6 mt-4 lg:mt-0">
                  <span className="font-accent text-[10px] tracking-[0.2em] font-medium text-deep-walnut uppercase">
                    Collection // {item.wood}
                  </span>
                  <h3 className="font-header text-4xl lg:text-7xl text-espresso text-balance leading-tight">
                    {item.name}
                  </h3>
                  <p className="font-french italic text-lg lg:text-3xl leading-tight text-espresso opacity-80 max-w-[280px] lg:max-w-none">
                    {item.tagline}
                  </p>

                  <Link to={`/product/${item.slug}`} className="mt-4 lg:mt-8 group flex items-center gap-4 text-espresso font-accent text-[10px] tracking-[0.2em] font-medium uppercase relative overflow-hidden px-6 py-4 border border-espresso/20 hover:border-espresso transition-colors">
