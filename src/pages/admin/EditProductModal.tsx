...
import { updateAdminProduct } from '../../lib/firestore';
import type { Product, ProductCategory, ProductCollection } from '../../data/products';
import type { AdminProduct } from '../../lib/firestore';
...
export default function EditProductModal({ product, onClose, onUpdated }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError('');
    try {
      const parsedPrice = parseInt(form.price.toString(), 10);
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-800 sticky top-0 bg-gray-900 z-10">
          <h2 className="text-sm text-gray-200 font-medium">Edit Product</h2>
          <button onClick={onClose} className="text-gray-500 hover:text-gray-300 transition-colors p-2 -mr-2">
            <X size={16} />
          </button>
        </div>
            ...
            <div
              ...
              className={`border border-dashed rounded-sm p-5 flex flex-col items-center justify-center text-center transition-all duration-200 ${
                uploading
                  ...
                  : 'border-gray-700 bg-gray-955 hover:bg-gray-900 hover:border-gray-600'
              } cursor-pointer`}
            >
              {uploading ? (
